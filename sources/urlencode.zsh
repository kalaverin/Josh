...
if [[ $(whence python3) != "" && ( "x$URLTOOLS_METHOD" = "x" || "x$URLTOOLS_METHOD" = "xpython" ) ]]; then
    ...
    alias urlencode='python2 -c "import sys, urllib as ul; print ul.quote_plus(\" \".join(sys.argv[1:]))"'
    alias urldecode='python2 -c "import sys, urllib as ul; print ul.unquote_plus(\" \".join(sys.argv[1:]))"'
# elif [[ $(whence php) != "" && ( "x$URLTOOLS_METHOD" = "x" || "x$URLTOOLS_METHOD" = "xphp" ) ]]; then
#     alias urlencode='php -r "echo rawurlencode(implode(\" \", \$argv)); echo \"\n\";"'
#     alias urldecode='php -r "echo rawurldecode(implode(\" \", \$argv)); echo \"\\n\";"'
# elif [[ $(whence node) != "" && ( "x$URLTOOLS_METHOD" = "x"  || "x$URLTOOLS_METHOD" = "xnode" ) ]]; then
