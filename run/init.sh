...
if [ -n "$(uname | grep -i freebsd)" ]; then
    export JOSH_OS="BSD"
    shortcut 'ls' '/usr/local/bin/gnuls' 1 >/dev/null
    shortcut 'grep' '/usr/local/bin/grep' 1 >/dev/null


elif [ -n "$(uname | grep -i darwin)" ]; then
    export JOSH_OS="MAC"
    shortcut 'ls' '/usr/local/bin/gls' 1 >/dev/null
    shortcut 'grep' '/usr/local/bin/ggrep' 1 >/dev/null
...
if [ "$JOSH_OS" = 'BSD' ] || [ "$JOSH_OS" = 'MAC' ]; then
    shortcut 'cut' '/usr/local/bin/gcut' 1 >/dev/null
    shortcut 'head' '/usr/local/bin/ghead' 1 >/dev/null
    shortcut 'readlink' '/usr/local/bin/greadlink' 1 >/dev/null
    shortcut 'realpath' '/usr/local/bin/grealpath' 1 >/dev/null
    shortcut 'sed' '/usr/local/bin/gsed' 1 >/dev/null
    shortcut 'tail' '/usr/local/bin/gtail' 1 >/dev/null
fi
